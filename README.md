...
## Extracted contents (for direct review)
The same package is also unzipped under [`extracted/`](extracted/) so every file
can be browsed and scanned individually, without downloading the archive. The
original `.zip` is kept unchanged alongside it (it is the file under review on
Nexus, so we are not modifying or removing it). The extracted tree is identical
to the archive — the only executable in it is `sekiro_boss_control.dll`; every
other file is plain text or Sekiro game-data (`.dcx` / `.hks` / `.ini` / `.txt`).
